...
for i in range(15):
    pedido = {}
    pedido["id"] = i
    pedido["createdAt"] = Date(random.randint(2018, 2021),random.randint(1, 12), random.randint(1, 28) )
    cantidad_productos = random.randint(1, 15)
    productos_pedido = []
    for i in range(cantidad_productos):
        id_producto = random.randint(0, len(productos)-1)
        producto = {}
        producto["id"] = id_producto
        producto["nombre"] = productos[id_producto]["title"]
        producto["categoria"] = productos[id_producto]["type"]
        producto["precio"] = productos[id_producto]["price"]
        producto["descripcion"] = productos[id_producto]["description"]
        producto["cantidad"] = random.randint(1, 10)
    ...
    if random.random() > 0.7:
      pedido["descuento"] = {"usa": True, "cupon": random.choice(cupones)} 
    else:
      pedido["descuento"] = {"usa": False, "cupon": False} 
    total = 0
    for p in pedido["productos"]:
      total += p["precioFinal"]
    pedido["precioFinalSinDcto"] = total
    if pedido["descuento"]["usa"]:
      descuento = int(pedido["descuento"]["cupon"]["descuento"])
      pedido["precioFinal"] = total*(1-descuento/100)
    else:
      pedido["precioFinal"] = total
    pedidos.append(pedido)
with open("src/data/lista_pedidos.json", "w") as file:
    json.dump(pedidos, file, default=str)